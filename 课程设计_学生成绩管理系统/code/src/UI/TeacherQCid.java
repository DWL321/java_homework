...
public class TeacherQCid extends JFrame {
	//整体布局
	private JPanel contentPanel = new JPanel();
	//按钮
	private JButton back = new JButton("返回"), go = new JButton("查询");
	//标题
	private JLabel	title = new JLabel("按课程号查询课程");
	//数据
	private String msg = "2;Math;19038024=cuizx,19308027=zhangsan,19802844=lisi";//这只是测试样例，真正的数据要从后端传过来的。
	private int cid;//需要传入后端的课程号。
	
	private JLabel jlb = new JLabel("请输入要查询的课程号: ");
	private JTextField jtf = new JTextField();		
	private int num;
	public TeacherQCid(int i) {
		this.num=i;
		this.init();
		this.addListener();
	}
	...
	private void addListener() {
		back.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				TeacherQCid.this.setVisible(false);
			}
		});
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(!tmp.equals("")) {
					...
					if(isNum.matches()) {
						...
						//数据传入后端操作
						Client.send.get(num).str="10:"+Client.id.get(num)+";"+cid;
	...
	public static void start(int i) {
		new TeacherQCid(i).setVisible(true);
	}
}