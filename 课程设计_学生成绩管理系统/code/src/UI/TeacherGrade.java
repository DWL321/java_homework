...
public class TeacherGrade extends JFrame{
	...
	//按钮
	private JButton qid = new JButton("按课程号查询"), lag = new JButton("按课程号修改"), back = new JButton("返回");
	//标题
	private JLabel	title = new JLabel("成绩管理页面");
	private int num;
	public TeacherGrade(int i) {
		this.num=i;
		this.init();
		this.addListener();
	}
	...
	private void addListener() {
		qid.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				TeacherQGid.start(num);
				TeacherGrade.this.setVisible(false);
			}
		});
		lag.addActionListener(new ActionListener() {
			//跳转到按课程号修改成绩界面。
			public void actionPerformed(ActionEvent e) {
				TeacherModifyG.start(num);
				TeacherGrade.this.setVisible(false);
			}
		});
	...
	public static void start(int i) {
		new TeacherGrade(i).setVisible(true);
	}
}