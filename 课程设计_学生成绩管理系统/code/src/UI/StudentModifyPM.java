...
public class StudentModifyPM extends JFrame {
	//整体布局
		...
		ok.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(status==1) {//在通过了前端校验的情况下。
					String passmsg ="5:"+Client.id.get(num)+";"+jtfname.getText() + ";" + t;
					//传入后端。
				...
				if(status==1) {
					...
					ModifyOK.start(0,num);
					StudentModifyPM.this.setVisible(false);
				}else {
					ModifyFalse.start(0,errmsg);
				}
			}
		});
	...
	public static void start(int i) {
		new StudentModifyPM(i).setVisible(true);
	}
}