...
public class RegistStatus extends JFrame{
	//整体布局
	private JPanel contentPanel = new JPanel();
	//按钮
	private JButton ok = new JButton("确定");
	private JLabel	title = new JLabel("注册失败！");
	private JLabel jlb = new JLabel("错误原因: ");
	private JTextArea jtf = new JTextArea();

	public RegistStatus(String errmsg) {
		this.init(errmsg);
		this.addListener();
	}
	...
	private void addListener() {
		ok.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				RegistStatus.this.setVisible(false);
			}
		});
	}
	
	public static void start(String errmsg) {
		new RegistStatus(errmsg).setVisible(true);
	}
}
